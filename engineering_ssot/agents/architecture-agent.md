...
3. **Audience Security Boundaries**: FastAPI `RoleChecker` must validate JWT `aud` claim (`aud: "patient"` vs `aud: "staff"`), blocking cross-audience access with HTTP 403.
4. **Pessimistic Locking**: New booking endpoints must follow the sequential row-level locking pattern (lock doctor shifts before checking/locking conflicting appointments).
5. **Data Isolation**: Verify that new data stores or tables respect the confidentiality (NDPR) and restricted medical classification rules, ensuring plaintexts are never visible to admins or database logs.
6. **UML Synchronization**: Any change to structural domain entities, backend logical services, scheduling workflows, verification paths, consultation routines, or scheduling lifecycle states must update the corresponding UML diagrams in `knowledge/architecture/UML/` (class, component, sequence, state, or activity diagrams) in sync with changes to code or requirements.
