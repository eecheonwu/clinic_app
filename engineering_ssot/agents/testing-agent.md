...
3. **Audience Boundary Enforcement**: Validate that `RoleChecker` denies patient JWTs on staff endpoints and staff JWTs on patient endpoints with HTTP 403.
4. **Failover Progression**: Notification worker integration tests must mock HTTP requests and simulate WhatsApp timeouts/failures to verify correct failover progression down to Termii and Infobip.
5. **Offline and Network Emulation**: Client-side UI testing must include network emulation verification (offline mode) to ensure IndexedDB/Dexie.js-cached data renders and that the offline warning banner is displayed.

## Test Status (2026-08-02)

### Test Summary
| Test Module | Count | Status |
|-----------|-------|--------|
| `tests/test_email_provider.py` | 16 | ✅ PASS |
| `tests/test_auth.py` | 55 | ✅ PASS |
| `tests/test_router_integration.py` | 16 | ✅ PASS |
| `tests/test_auth_patient.py` | 53 | ✅ PASS |
| `tests/test_jwt_audience.py` | 7 | ✅ PASS |
| `tests/integration/test_email_registration_flow.py` | 2 | ✅ PASS |
| `tests/integration/test_login_separation.py` | 3 | ✅ PASS |
| **Total Backend Test Suite** | **152** | **100% PASS** |
...
- JWT Audience Isolation: Verified `aud: "patient"` vs `aud: "staff"` ✅
- Password Policy: Min 8 chars, 1 upper, 1 lower, 1 digit, 1 special char ✅
- Resend Verification Rate Limit: 3 requests per 15 minutes ✅