...
| email | VARCHAR(255) UNIQUE | NOT NULL (Phase 1 Phase-in) |
...
### `email_verification_tokens` — Internal
| Column | Type | Notes |
|---|---|---|
| id | UUID PK | `gen_random_uuid()` |
...
| expires_at | TIMESTAMPTZ | 60-minute TTL |
...
|---|---|---|
| id | UUID PK | |
| user_id | UUID | Nullable (no FK — immutable) |
...
| email_verification_tokens | `ix_email_verification_tokens_email` | email | Token lookups by recipient email |
| email_verification_tokens | `ix_email_verification_tokens_expires_at` | expires_at | Token expiration cleanup queries |
| security_audit_logs | `ix_security_audit_logs_user_timestamp` | user_id, timestamp | Audit queries |
| clinical_records | `ix_clinical_records_patient_id` | patient_id | Patient record lookups |
| clinical_records | `ix_clinical_records_appointment_id` | appointment_id | Appointment joins |