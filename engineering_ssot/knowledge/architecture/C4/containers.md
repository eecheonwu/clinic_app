...
| Celery Worker Instances | Celery + Python | Async background task processors (notifications, OTP delivery, auth email delivery) |
| Redis Queue | Redis | In-memory broker for background task queuing |
| PostgreSQL Database | PostgreSQL 16+ (AWS RDS) | ACID scheduling database, audit logs, clinical records, verification tokens |
...
| Email Gateway API | External (SMTP/SendGrid/SES) | Verification email & authentication link delivery |
...
[FastAPI Application Server]
    ├──→ [PostgreSQL] (reads/writes/pessimistic locks/tokens)
    ...
    └──→ [Redis Queue]
        ...
        [Celery Workers]
            ...
            └──→ [Email Gateway API] (auth & notification emails)
```
...
graph TB
    ...
    subgraph External ["External Integration API Tier"]
        ...
        EmailAPI["Email Gateway API (SMTP/SendGrid/SES)"]
    end
    ...
    Workers -->|Sends auth emails| EmailAPI
```
