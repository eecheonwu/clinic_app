...
- **Purpose**: Validates JWT tokens with audience differentiation (`aud: "patient"` vs `aud: "staff"`) and enforces role-based access control.
- **Technology**: FastAPI Security Scopes, JWT (OAuth2 Bearer)
- **API surface**: 
  - Staff: `POST /api/v1/auth/login` (Restricted to staff roles)
  - Patient: `POST /api/v1/auth/patient/register`, `POST /api/v1/auth/patient/verify-email`, `POST /api/v1/auth/patient/login`, `POST /api/v1/auth/patient/resend-verification`
  - General: `GET /api/v1/me`
  - Deprecated (OTP): `POST /api/v1/register`, `POST /api/v1/verify-request`, `POST /api/v1/verify-code` (Return `Deprecation: true` headers)
- **Dependencies**: `users` table, `email_verification_tokens` table, `verification_otps` table, `patient_profiles` table

### PatientAuth & Email Verification Service
- **Purpose**: Manages email-based patient registration, verification token generation, password validation, and separate patient login.
- **Technology**: Python, bcrypt token hashing, Celery (for async auth emails), Redis (rate limiting 3 requests/15min)
- **Business rules**: 60-min token TTL, single-use tokens, bcrypt hashed storage, min 8 char password policy, JWT `aud: "patient"` issuance
- **Dependencies**: `users` table, `email_verification_tokens` table, `patient_profiles` table, Notification Service (Email adapter)
...
| Email Provider (SMTP / SendGrid / AWS SES) | SMTP / REST (HTTPS) | Auth verification emails & password creation links (ADR-005) | Outbound |
...
- **Response 201**: `{ message: "OTP code generated and sent to target phone number...", registration_token: "jwt-string" }`
- **Behaviour**: Validates phone uniqueness, generates 6-digit OTP code, enqueues delivery via Notification Engine (WhatsApp/SMS), and returns a signed `registration_token`. Patient data is NOT persisted to the database until OTP verification.
...
- **Access**: Public
- **Request**: `{ phone_number, otp_code, registration_token?, registration_data? }`
- **Response 200**: `{ access_token, refresh_token, token_type, expires_in }`
- **Behaviour**: Validates 6-digit OTP code against `verification_otps`. Upon successful verification, decodes pending registration token/payload to create patient `User` and `PatientProfile` in DB (if registering), and returns fresh access and refresh JWT tokens.
...
### Patient Auth API Contracts

#### POST /api/v1/auth/patient/register
- **Access**: Public (Rate limited: 3 req/15min)
- **Request**:
  ```json
  {
  ...
  ```
- **Response 200**: `{ "message": "Verification email sent. Please check your inbox to create your password." }`
- **Errors**: 400 (validation), 409 (email/phone already registered), 429 (rate limit exceeded)
...
- **Request**:
  ...
  ```
- **Response 200**: `{ "access_token": "jwt", "refresh_token": "jwt", "token_type": "bearer", "user": { ... } }` (JWT includes `aud: "patient"`)
- **Errors**: 400 (invalid token, expired token, password policy failure), 409 (token already used)
...
- **Request**:
  ...
  ```
- **Response 200**: `{ "access_token": "jwt", "refresh_token": "jwt", "token_type": "bearer" }` (JWT includes `aud: "patient"`)
- **Errors**: 401 (invalid credentials), 403 (email not verified)

#### POST /api/v1/auth/patient/resend-verification
- **Access**: Public (Rate limited: 3 req/15min)
- **Request**: `{ "email": "patient@example.com" }`
- **Response 200**: `{ "message": "New verification link sent to your email." }`
- **Errors**: 409 (email already verified), 429 (rate limit)