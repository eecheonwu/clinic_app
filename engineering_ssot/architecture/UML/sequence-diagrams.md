...
## 4. Email Patient Registration, Verification & Password Creation Flow

Visualizes the email-based patient registration sequence (**ADR-005**), generating cryptographically secure verification tokens, sending auth emails, and creating passwords.

```mermaid
sequenceDiagram
    autonumber
    actor Patient as Patient Browser
    participant API as FastAPI Backend (PatientAuth)
    participant DB as PostgreSQL DB
    participant MQ as Task Queue (Celery/Redis)
    participant Email as Email Provider (SMTP/SES)

    Note over Patient, Email: Step 1: Patient Self-Registration Request
    Patient->>API: POST /api/v1/auth/patient/register (email, phone, profile_data)
    API->>DB: Check if email or phone already exists in users table
    alt Email or Phone Exists
        DB-->>API: Conflict found
        API-->>Patient: HTTP 409 Conflict ("Email or phone already registered")
    else New Registration
        API->>API: Generate cryptographically random token (secrets.token_urlsafe(32))
        API->>API: Hash raw token using bcrypt
        API->>DB: UPDATE email_verification_tokens SET is_expired=TRUE WHERE email={email}
        API->>DB: INSERT INTO email_verification_tokens (email, token_hash, expires_at=60min)
        API->>MQ: Enqueue send_auth_email task (email, raw_token)
        API-->>Patient: HTTP 200 OK {"message": "Verification email sent"}
    end

    MQ->>Email: Send Auth Email with link ?token={raw_token}
    Email-->>Patient: Verification Email Delivered

    Note over Patient, DB: Step 2: Patient Clicks Link & Sets Password
    Patient->>API: POST /api/v1/auth/patient/verify-email (raw_token, password, confirm_password)
    API->>API: Validate password complexity rules (min 8 chars, upper, lower, digit, special)
    API->>DB: SELECT active email_verification_tokens WHERE email={email} AND is_used=FALSE AND is_expired=FALSE
    API->>API: Compare raw_token with token_hash via bcrypt
    alt Token Invalid, Expired, or Used
        API-->>Patient: HTTP 400 Bad Request / 409 Conflict ("Token invalid or expired")
    else Token Valid
        API->>DB: BEGIN Transaction
        API->>DB: UPDATE email_verification_tokens SET is_used=TRUE WHERE id={token_id}
        API->>DB: INSERT INTO users (email, phone, password_hash, role='patient', is_email_verified=TRUE, email_verified_at=NOW())
        API->>DB: INSERT INTO patient_profiles (user_id, full_name, date_of_birth, gender, emergency_contact)
        API->>DB: COMMIT Transaction
        API->>API: Generate Patient JWT Access & Refresh Tokens (aud: "patient")
        API-->>Patient: HTTP 200 OK {"access_token": jwt, "user": {...}}
    end
```

---

## 5. Audience-Differentiated Patient vs Staff Auth & Route Guarding Flow

Visualizes JWT audience validation (`aud: "patient"` vs `aud: "staff"`) and route access control (**ADR-005**).
...
sequenceDiagram
    ...
    participant API as FastAPI Backend (RoleChecker)
    participant DB as PostgreSQL DB

    Note over Patient, API: Patient Login Flow
    Patient->>API: POST /api/v1/auth/patient/login (email, password)
    API->>DB: SELECT user WHERE email={email} AND role='patient'
    API->>API: Verify bcrypt password & is_email_verified==TRUE
    API->>API: Generate JWT with claim aud: "patient", role: "patient"
    API-->>Patient: HTTP 200 OK (access_token with aud: "patient")

    Note over Doctor, API: Staff Login Flow
    Doctor->>API: POST /api/v1/auth/login (email, password)
    API->>DB: SELECT user WHERE email={email} AND role IN ('doctor', 'receptionist', 'manager', 'admin')
    API->>API: Verify bcrypt password
    API->>API: Generate JWT with claim aud: "staff", role: "doctor"
    API-->>Doctor: HTTP 200 OK (access_token with aud: "staff")

    Note over Patient, API: Cross-Audience Route Protection Test
    Patient->>API: GET /api/v1/clinical-records/patient/123 (Header: Bearer PatientJWT [aud: "patient"])
    API->>API: RoleChecker evaluates endpoint requirements (Requires aud: "staff", role: "doctor")
    API->>API: Mismatch detected (Token aud is "patient", expected "staff")
    API-->>Patient: HTTP 403 Forbidden ("Access denied: Staff credentials required")
```
