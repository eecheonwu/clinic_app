...
| Authentication & RBAC Manager | FastAPI Security Scopes | Validates JWTs (`aud: patient` vs `aud: staff`); enforces user role permissions |
| PatientAuth Service | Python / FastAPI | Handles patient registration, email verification, and separate patient login flows |
| EmailVerification Engine | Python | Manages cryptographically secure single-use 60-min verification tokens |
...
| Notification Service Abstraction | Python / Celery | Enqueues async alert & email dispatches; implements Strategy Pattern failover |
...
[Auth & RBAC Manager] ←→ [API Route Controllers]
        ↓
┌──────────────────────────────────────────┐
│  [PatientAuth Service]                   │──→ [EmailVerification Engine] → [PostgreSQL]
│  [EmailVerification Engine]              │──→ [PostgreSQL]
...
graph TB
    ...
    subgraph FastAPIContainer ["FastAPI Backend Modules"]
        ...
        Auth["Authentication & RBAC Manager\nFastAPI Security Scopes\nValidates JWTs (aud: patient vs staff)"]
        PatientAuth["PatientAuth Service\nPatient email registration & separate login"]
        EmailVerification["EmailVerification Engine\nGenerates & validates bcrypt hashed tokens"]
    ...
    PatientAuth --> EmailVerification
    PatientAuth -->|Writes users & profiles| PostgreSQL[("PostgreSQL DB")]
    EmailVerification -->|Manages verification tokens| PostgreSQL
    Scheduler -->|Pessimistic transactions| PostgreSQL
    ...
    NotificationPublisher -->|Push async tasks & email tasks| RedisQueue["Redis Task Queue"]
...
|---|---|---|
| `users` | Auth (unencrypted) | id (PK), phone_number (UK), email (UK), password_hash, role, is_email_verified, email_verified_at |
| `email_verification_tokens` | Internal | id (PK), email, token_hash (bcrypt), attempts, is_used, is_expired, expires_at, created_at |
...
- `users` (email) 1 → 0..* `email_verification_tokens`
...
erDiagram
    ...
    users ||--o{ email_verification_tokens : "receives verification tokens"
